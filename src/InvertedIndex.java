...
public class InvertedIndex {
	...
	/**
	 * Adds the word, the file it appears in, and the position it appears in to the inverted index
	 * @param word word inside of the file
	 * @param location path of the file
	 * @param position position in which the word appears in the file
	 */
	public void add(String word, String location, int position) {
		if (index.containsKey(word)) {
			if (index.get(word).containsKey(location)) {
				if (!index.get(word).get(location).contains(position)) {
					index.get(word).get(location).add(position);
				}
			} else {
				index.get(word).put(location, new TreeSet<Integer>());
				index.get(word).get(location).add(position);
			}
		} else {
	 ...
	 */
	public boolean containsPath(String word, String location) {
		if (containsWord(word)) {
	 ...
	 * @param location file path the word is in
	 ...
	 * @param location path to the file to write to
	 * @throws IOException in case there's any problem finding the file
	 */
	public void writeIndexJSON(Path location) throws IOException {
		TreeJSONWriter.asTripleNested(this.index, location);
	}
